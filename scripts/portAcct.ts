...
yargs(hideBin(process.argv))
  .options({ user: { type: 'string', default: 'devjimmyboy', description: 'Specify a user to update' } })
  .command('port', "Convert a user's data")
  .command('wipe', "wipe a user's data")
  .parseAsync()
  .then((flags) => {
    user = flags.user
    if (flags._[0] === 'wipe') wipe()
    else if (flags._[0] === 'port') port()
  })
