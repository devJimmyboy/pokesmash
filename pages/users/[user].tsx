...
const UserProfile: NextPage<Props> = ({ user }) => {
  ...
  const { data: score, error: scoreError } = useSWR<ScoreProp, ScoreError>(
    typeof user === "object" &&
      `/api/user/score?user=${user.name.toLowerCase()}`,
    fetcher
  );
      ...
      <>
        ...
        >
          <Tooltip className="absolute top-12 md:top-24 left-4" title="Go Back">
            <IconButton
              className="fancy-bg w-10 h-10 p-0 m-0"
              LinkComponent={Link}
              href="/"
            >
          ...
          <div className="flex flex-row gap-6 items-center w-11/12 my-2 md:my-4 lg:my-6 ">
            ...
            >
              <IconButton
                ...
                onClick={() =>
                  router.push(
                    `https://twitch.tv/${user?.displayName}`,
                    {},
                    { shallow: true }
                  )
                }
          ...
          >
            <StyledTab
              icon={
                <Image
                  src="https://cdn.7tv.app/emote/60aeafcb229664e866bef5ac/4x"
                  alt="PogO"
                  width={32}
                  height={32}
                />
            ...
            <StyledTab
              icon={
                <Image
                  src="https://cdn.7tv.app/emote/611a4aac62a016377dd91a25/4x"
                  alt="peepoG"
                  width={32}
                  height={32}
                />
            ...
            <StyledTab
              icon={
                <Image
                  src="https://cdn.7tv.app/emote/60b8cce455c320f0e89d3514/4x"
                  alt="EZ"
                  width={32}
                  height={32}
                />
          ...
          >
            {!scoreError ? (
              tab === 1 ? (
                <StatsPage
                  smashes={score.smashCount}
                  passes={score.passCount}
                />
              ) : (
                <div className="absolute h-full w-full overflow-y-auto">
                  ...
                  >
                    {Object.keys(score.choices)
                      .filter(
                        (val) =>
                          score.choices[val] === (tab === 2 ? "smash" : "pass")
                      )
                      .map((id) => (
                        <Grid item xs={2} sm={3} md={3} lg={4} key={id}>
                          <PokemonSquare
                            i={Number(id)}
                            choice={tab === 2 ? "smash" : "pass"}
    ...
    <div className="w-full h-full flex flex-col items-center justify-center gap-12">
      ...
      <div className="flex flex-row justify-around w-full">
        <div className="flex flex-col gap-4 items-center">
          ...
          <ScoreDisplay className="passes p-12 text-5xl">
            {props.passes}
          </ScoreDisplay>
        </div>
        ...
        <div className="flex flex-col gap-4 items-center ">
          ...
          <ScoreDisplay className="smashes p-12 text-5xl">
            {props.smashes}
          </ScoreDisplay>
        </div>
      </div>
