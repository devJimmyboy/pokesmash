...
const UserProfile: NextPage<Props> = ({ user }) => {
  ...
  useEffect(() => {
    if (pictureBgRef.current) {
      tl.fromTo(pictureBgRef.current, { scale: 1, autoAlpha: 1, borderWidth: 8 }, { scale: 1.25, autoAlpha: 0, duration: 2, borderWidth: 0, ease: 'cubic.bounce' })
      tl.play()
    }
  }, [pictureBgRef])
    ...
    return (
      <>
        <Head>
          ...
          <meta name="description" content={`The user page for ${user.name}. Shows the Smashes and Passes they chose.`} />
        </Head>
        <Stack className="poke-scrollbar" sx={{ overflow: 'hidden', height: '100vh' }} direction="column" p={6} pb={2} alignItems="center">
          ...
          <StyledTabs value={tab} variant="fullWidth" onChange={(e, nTab) => setTab(nTab)} sx={{ width: '100%' }} aria-label="User's Stats, Smash list, and Pass list.">
            <StyledTab icon={<Image src="https://cdn.7tv.app/emote/60aeafcb229664e866bef5ac/4x" width={32} height={32} />} label="Passes" iconPosition="start" />
            <StyledTab icon={<Image src="https://cdn.7tv.app/emote/611a4aac62a016377dd91a25/4x" width={32} height={32} />} label="Stats" iconPosition="start" />
            <StyledTab icon={<Image src="https://cdn.7tv.app/emote/60b8cce455c320f0e89d3514/4x" width={32} height={32} />} label="Smashes" iconPosition="start" />
          </StyledTabs>
          <div
